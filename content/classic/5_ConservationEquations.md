...
S' rightfully concludes, based on Newton's 1$^{st}$ law that no force is acting on $m$. S agrees, but doesn't conclude that $m$ is at rest. This is trival: both observers can use Newton's second law which for this case states that $\frac{d\vec{p}}{dt} =0 \rightarrow \vec{p} = const \rightarrow \vec{v} = const$. But the constant is not the same in both frames.
